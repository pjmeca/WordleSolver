...
public class WordEvaluator {
	...
	private static final String DEFAULT_FREQUENCY_RUTE = "resources/spanish/frequency.txt";
	...
	public int eval(String word) {
		...
		int repeatedValue = 0;
		if(repeatedPercent >= 0.5) repeatedValue = 0;
		else repeatedValue = 10 - (int) Math.round(repeatedPercent*10);
		
		// Different vowels -> good
																// wont take into account accent marks
		double numVowels = (double) (letters.stream().filter(c -> Pattern.matches("[aeiou]", c+"")).collect(Collectors.toSet()).size());
